...
def get_conf_path():
    ...
    path = os.path.dirname(module.__file__)+r"/.."
    # print path,"<<<<<<"
...
def read_sourcetype_conf():
    global gloabl_V
    # file_path=get_conf_path()
    # # file_path=r'/Volumes/Transcend/splunk/etc/system/default/sourcetypes.conf'
    # f=open(file_path)
    # content=f.read()
    content = SOURCE_TYPE_CONF_CONTENT
    # f.close()
    ret=[]
...
def _preprocess(text_list):
    ...
    for line in text_list:
        punc= _get_punc_string1(line)
        if ret.has_key(punc):
            ret[punc]+=1
        else:
            ret[punc]=1
    # print text,len(text)
    s_list= _remove_stop_word(text_list)
    if _remove_number==1:
        s_list=remove_number(s_list)
    content_dict= _count_words(s_list)
    if _debug:
        print ret
...
def _count_words(list_content):#统计给定的text中的单词频数
    text_count={}
...
def compute_prior(different_source_type_dis):
    ...
    total_count= _count_words(source_type_vector)
    ttt= sum(total_count.values())
    for x in total_count:
        print x,float(total_count[x])/ttt

def _get_score(source_type_dict, content_dict):
    ret=[]
...
def predict_source_type(content_dict,different_source_type_dis):
    ...
    for source_type in different_source_type_dis:
        ...
        pred["score_vector"]= _get_score(source_type, content_dict)
        pre.append(pred)
...
def _use_reg_match_source_type(content_text_list, reg_for_match):
...
def _use_upload_match_sourcetype(upload_path, upload_for_match):
    for key in upload_for_match:
        if re.match(upload_for_match[key],upload_path)!=None:
            return upload_for_match[key]
    return False

def try_to_detect_file_sourcetype(text_list,upload_path=None):


    """
    :param text_list: a list contains raw line
    :param upload_path: the absolute path which file in on user localtion
    :return: if find a  soucetype return it ,else return None
    """
    # 1:regular match
    # 2:upload path
    # 3:bayes
    ...
    if upload_path is not None:
        ...
        sourcetype= _use_upload_match_sourcetype(upload_path, upload_for_match)
    if sourcetype:
        if _debug:
            print "upload match!"
        return sourcetype
        ...
        # f.close()
        content_text_list=text_list
        sourcetype= _use_reg_match_source_type(content_text_list, reg_for_match)
        if sourcetype:
            if _debug:
                print "regular match"
            return sourcetype
            ...
            content_dict= _preprocess(text_list)
...
if __name__ == "__main__":
    logfile = "/Users/wanghe/LogSample/apache/cplog/access_log-20150201"
    upload_path = "/var/log/apache/access.log"
    fd = open(logfile, "r")
    lines = fd.readlines()
    fd.close()
    ret = try_to_detect_file_sourcetype(lines, upload_path)
    print ret
